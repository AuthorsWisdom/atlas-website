...
export default function Nav() {
  function enterLink(e: React.MouseEvent<HTMLAnchorElement>) {
    e.currentTarget.style.color = 'var(--text)'
  }
  function leaveLink(e: React.MouseEvent<HTMLAnchorElement>) {
    e.currentTarget.style.color = 'var(--text-2)'
  }
  function enterBtn(e: React.MouseEvent<HTMLAnchorElement>) {
    e.currentTarget.style.opacity = '0.85'
  }
  function leaveBtn(e: React.MouseEvent<HTMLAnchorElement>) {
    ...
    <nav style={{ position: 'sticky', top: 0, zIndex: 100, backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', background: 'rgba(8,8,8,0.8)', borderBottom: '1px solid var(--border)' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 2rem', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: '2rem', alignItems: 'center' }}>
          ...
          <a href="#waitlist" style={{ fontSize: '13px', fontWeight: 500, color: 'var(--bg)', background: 'var(--text)', padding: '7px 16px', borderRadius: '6px', transition: 'opacity 0.2s', textDecoration: 'none' }} onMouseEnter={enterBtn} onMouseLeave={leaveBtn}>Early access</a>
        </div>
